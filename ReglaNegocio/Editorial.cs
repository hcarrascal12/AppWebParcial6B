...
namespace ReglaNegocio
{
    public class Editorial
    {
        ...
        {
            var vSql = "SELECT [Id], [Descripcion] FROM editorial WHERE estado = 'A'";
            var gv = new Grilla();
        ...
        {
            int numReg = 0;
            var vSql = "insert into editorial ([Descripcion], [Estado]) values (?,?)";
            bd.Conectar();
        ...
        {
            int numReg = 0;
            var vSql = "UPDATE Editorial SET [Descripcion] = ? WHERE [ID] = ?";
            bd.Conectar();
            bd.CrearComando(vSql, CommandType.Text);
            bd.AsignarParametro("?", OleDbType.VarChar, this.Descripcion);
        ...
        {
            int numReg = 0;
            var vSql = "UPDATE Editorial SET [estado] = 'I' WHERE [ID] = ?";
            bd.Conectar();
            bd.CrearComando(vSql, CommandType.Text);
            bd.AsignarParametro("?", OleDbType.Integer, this.id);
            numReg = bd.EjecutarComando();
